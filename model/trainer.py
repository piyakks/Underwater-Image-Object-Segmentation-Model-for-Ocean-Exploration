...
class XMemTrainer:
    ...
    def load_network_in_memory_control(self, src_dict):
        self.XMem.module.load_weights_control(src_dict)
        print('Network weight loaded from memory.')
        trainable_params = sum(p.numel() for p in self.XMem.parameters() if p.requires_grad)
    ...
    def load_network_control(self, path):
        # This method loads only the network weight and should be used to load a pretrained model
        map_location = 'cuda:%d' % self.local_rank
        src_dict = torch.load(path, map_location={'cuda:0': map_location})

        self.load_network_in_memory_control(src_dict)
        print(f'Network weight loaded from {path}')
