...
from util.load_subset import load_sub_davis, load_sub_yv,load_water
...
for si, stage in enumerate(stages_to_perform):
    ...
    def renew_vos_loader(max_skip, finetune=False):
        ...
        water_vase = VOSDataset(path.join(water_root, 'JPEGImages', '480p'), 
                            path.join(water_root, 'Annotations', '480p'), max_skip, is_bl=False, subset=load_water(), num_frames=config['num_frames'], finetune=finetune)
        train_dataset = ConcatDataset([water_vase] + [water_vase])
    ...
    if stage == '0':
        ...
        water_root = path.join(path.expanduser(config['water_root']), 'train', 'oh')
